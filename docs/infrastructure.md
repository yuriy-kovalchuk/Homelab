...
The cluster is managed via Terraform located in `terraform/clusters/main/`:

```
terraform/clusters/main/
...
├── machine_config_patches/  # Talos machine config templates
└── vars/terraform.tfvars    # Variable values
```
...
Maya infrastructure is managed via Terraform in `terraform/infrastructure/maya/`:

```
terraform/infrastructure/maya/
...
├── variables.tf     # VM specs and ACME variables
├── backend.tf       # S3 backend configuration
└── outputs.tf       # Terraform outputs
```

The Talos cluster configuration (secrets, bootstrap) is separate in `terraform/clusters/management/`.
...
Application-specific configurations are in `terraform/apps/`:

| Module | Purpose |
|--------|---------|
| `terraform/apps/vault` | Manage secrets in HashiCorp Vault |
| `terraform/apps/authentik` | Configure OIDC providers for SSO |
...
|----------|---------|---------|
| `bpg/proxmox` | 0.93.0 | Proxmox VM management |
| `siderolabs/talos` | 0.9.0 - 0.10.0 | Talos Linux configuration |
| `hashicorp/vault` | ~> 4.0 | Vault secrets management |
| `goauthentik/authentik` | 2025.8.1 | Authentik OIDC configuration |
| `hashicorp/helm` | ~> 2.0 | Helm chart deployments |
| `hashicorp/kubernetes` | ~> 2.0 | Kubernetes resources |

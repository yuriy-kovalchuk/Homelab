...
variable "lan_bridge" {
  description = "LAN network bridge (internal homelab network, 10.0.0.0/16)"
  type        = string
  default     = "vmbr1"
}

variable "lan_bridge_port" {
  description = "Physical NIC to attach to the LAN bridge (e.g. enp3s0, eth1)"
  type        = string
}

variable "opnsense_iso_file_id" {
  description = "Proxmox storage path to the uploaded OPNsense ISO (e.g. local:iso/OPNsense-26.1.6-dvd-amd64.iso)"
  type        = string

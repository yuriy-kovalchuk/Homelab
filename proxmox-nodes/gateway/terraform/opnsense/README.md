...
- `TF_VAR_proxmox_endpoint` — Proxmox API URL (e.g. `https://192.168.0.254:8006/`)
- `TF_VAR_proxmox_password` — Proxmox root password
- `TF_VAR_lan_bridge_port` — physical NIC to attach to `vmbr1` (run `ip link show` on Proxmox to find it)
- `TF_VAR_opnsense_iso_file_id` — storage path of uploaded ISO (e.g. `local:iso/OPNsense-26.1.6-dvd-amd64.iso`)
...
- OPNsense VM (ID 1002) with WAN on `vmbr0` and LAN on `vmbr1`

## 4. Fix boot order

After `tf_apply`, the VM boots from the ISO by default. In Proxmox web UI:
...
VLANs are configured in OPNsense via:
**Interfaces → Other Types → VLAN → add `vtnet1.{vlan-id}`** for each VLAN (1–6).

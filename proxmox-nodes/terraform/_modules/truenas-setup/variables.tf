...
variable "truenas_pool_disks" {
  description = "List of disk identifiers to include in the pool (e.g. [\"sdb\"])"
  type        = list(string)
}
...
variable "truenas_nas_username" {
  description = "Username for the NAS SMB/NFS user"
  type        = string
}

variable "truenas_nas_user_password" {
  description = "Password for the NAS SMB/NFS user"
  type        = string
  sensitive   = true
}

# Management Cluster Platform

This directory manages the platform services for the Management cluster, including Cilium, Harbor, Vault, and Cert-Manager.

## Architecture

Platform components are managed using a mix of the shared modules and local resources. Helm configurations are standardized to use `values.yaml` or `.tftpl` files in the `helm_values/` directory.
